 ...
 *  - Techo del Competitivo usa la racha MÁXIMA de la temporada (no la actual).
...
function getAllData() {
  ...
  const chunksStr = cache.get('allData_chunks');
  if (chunksStr) {
    try {
      const chunks = parseInt(chunksStr);
      let json = '';
      for (let i = 0; i < chunks; i++) {
        const chunk = cache.get('allData_' + i);
        if (!chunk) break;
        json += chunk;
      }
  ...
  try {
    ...
    if (json.length < 100000) {
      cache.put('allData', json, 21600);
    } else {
      const chunkSize = 90000;
      const chunks = Math.ceil(json.length / chunkSize);
      const keys = {};
      for (let i = 0; i < chunks; i++) {
        keys['allData_' + i] = json.substr(i * chunkSize, chunkSize);
      }
      keys['allData_chunks'] = String(chunks);
...
function refreshCache() {
  ...
  for (let i = 0; i < 20; i++) cache.remove('allData_' + i);
  cache.remove('allData_chunks');
  getAllData();
...
function buildAllData() {
 ...
 // ── COMPETITIVO (leído de la pestaña "Ranking Competitivo" ya calculada) ──
  const eligible = rappers.filter(r => r.ev >= 8);
  ...
  if (compSheet) {
    ...
    const compMap = {};
    if (cHdr !== -1) {
      // Columnas: A=# B=Rapero C=Sv D=Ev E=Score F=Confianza
      //           G=Eficiencia H=Consistencia I=Dominancia J=Techo K=Diversidad
      for (let i = cHdr + 1; i < compData.length; i++) {
        const row = compData[i];
        const name = String(row[1] || '').trim();
        if (!name || row[0] === '' || row[0] === null) continue;
        compMap[name] = {
          comp_pos: parseNum(row[0]),
          comp_score: parseNum(row[4]),
          comp_confidence: parseNum(row[5]),
          comp_dims: {
            efficiency:  parseNum(row[6]),
            consistency: parseNum(row[7]),
            dominance:   parseNum(row[8]),
            ceiling:     parseNum(row[9]),
            diversity:   parseNum(row[10])
          }
        };
      }
    }
