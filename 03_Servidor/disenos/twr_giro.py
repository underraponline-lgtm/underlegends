"""TWR · girar las franjas para el otro lado.

El actual va a 126deg (baja de izquierda a derecha). El espejo es 234deg.
Se prueban ademas distintas inclinaciones y donde queda el fantasma.
"""
...
def franjas(ang):
    return (f'linear-gradient({ang}deg,{t(A,-.52)} 0 30%,{A} 30% 46%,'
            f'{t(B,-.15)} 46% 72%,{t(A,-.4)} 72% 100%)')


FANTASMA = ('background-image:url(SIL);background-size:128%;'
            'background-position:POS;background-repeat:no-repeat;'
            'opacity:.26;mix-blend-mode:overlay').replace('SIL', SIL)

CASOS = [
 ('1 · el actual · 126°',        franjas(126), 'center 74%'),
 ('2 · espejo · 234°',           franjas(234), 'center 74%'),
 ('3 · espejo más parado · 215°',franjas(215), 'center 74%'),
 ('4 · espejo más tumbado · 252°',franjas(252),'center 74%'),
 ('5 · espejo + fantasma a la izquierda', franjas(234), '22% 72%'),
 ('6 · espejo + fantasma a la derecha',   franjas(234), '78% 72%'),
]


def carta(etq, fondo, pos):
...
<div class="wrap"><div class="marco"><div class="panel">
  ...
  <div class="cap" style="{FANTASMA.replace('POS', pos)}"></div>
...
.marco{{position:absolute;inset:0;clip-path:{ESCUDO};background:{t(A,-.64)};
...
.foto{{position:absolute;top:12.5%;left:50%;transform:translateX(-50%);width:36%;height:23%;
  border-radius:13px;overflow:hidden;z-index:4;border:2.5px solid {B};
...
.rng{{font-family:'Archivo','LigaEmoji',sans-serif;font-size:.76rem;font-weight:900;color:{B};line-height:1;
...
.gema{{position:absolute;left:50%;bottom:1%;transform:translateX(-50%) rotate(45deg);
  width:19%;height:19%;z-index:8;border-radius:6px;background:{t(A,-.48)};
  box-shadow:0 5px 15px rgba(0,0,0,.9),inset 0 0 0 2px {B};
...
async def main():
    ...
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           ...
           '<div class="rot">TWR · GIRAR LAS FRANJAS <span>— el actual va a 126°, '
           'el espejo es 234°</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'twr_giro.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'twr_giro.png'), full_page=True)
        await b.close()
    print('-> twr_giro.png')
