"""TWR con el sangrado diagonal elegido. La pregunta: hace falta la textura?

Se aisla SOLO eso. El fondo es identico en las seis; lo unico que cambia es
como aparece la silueta de TWR:
  · grano     = la silueta en mosaico chico, repetida
  · fantasma  = la silueta grande, una sola, de marca de agua
"""
...
o = t(A, -.52)
FONDO = (f'background:linear-gradient(126deg,{o} 0 30%,{A} 30% 46%,'
         f'{t(AC,-.15)} 46% 72%,{t(A,-.4)} 72% 100%)')

# (etiqueta, css del grano, css del fantasma)
CASOS = [
 ('A · como esta (grano + fantasma)',
  f'background-image:url({SIL});background-size:34px;opacity:.20;mix-blend-mode:soft-light',
  f'background-image:url({SIL});background-size:106%;background-position:center 40%;'
  'background-repeat:no-repeat;opacity:.16;mix-blend-mode:overlay'),

 ('B · solo el fantasma grande', 'display:none',
  f'background-image:url({SIL});background-size:106%;background-position:center 40%;'
  'background-repeat:no-repeat;opacity:.18;mix-blend-mode:overlay'),

 ('C · solo el grano en mosaico',
  f'background-image:url({SIL});background-size:34px;opacity:.22;mix-blend-mode:soft-light',
  'display:none'),

 ('D · sin nada, limpio', 'display:none', 'display:none'),

 ('E · fantasma fuerte y bajo', 'display:none',
...
def carta(etq, grano, fantasma):
    return f"""<div class="col"><div class="et">{etq}</div>
<div class="wrap">
  <div class="marco"><div class="panel">
    <div class="fondo" style="{FONDO}"></div>
    <div class="cap" style="{grano}"></div>
    <div class="cap" style="{fantasma}"></div>
...
async def main():
    ...
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           ...
           '<div class="rot">TWR · SANGRADO DIAGONAL <span>— hace falta la textura de TWR atrás? '
           'El fondo es idéntico en las seis; lo único que cambia es la silueta.</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'twr_textura.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'twr_textura.png'), full_page=True)
        await b.close()
    print('-> twr_textura.png')
