...
DRA es el servidor de Under Legends: micros cruzados y CORONA, azul vivo y
blanco. Realeza, pero moderna, no ornamental como Fontana.

Aprendido en URBF: el degrade FUERTE le da profundidad a un fondo que solo
con color queda plano. Varias de estas lo usan.
"""
...
FONDOS = [
 ('1 · el actual · placa lisa',
  f'linear-gradient(168deg,{c} 0%,{t(A,-.30)} 100%)'),

 ('2 · corona · rayos desde arriba',
  'repeating-conic-gradient(from 200deg at 50% -8%,'
  'rgba(255,255,255,.14) 0deg 4deg,transparent 4deg 11deg),'
  f'radial-gradient(ellipse 90% 50% at 50% -4%,{t(A,.42)},transparent 68%),'
  + FUERTE),

 ('3 · manto · degradé profundo',
  FUERTE),

 ('4 · chevron de realeza',
  'linear-gradient(135deg,rgba(255,255,255,.13) 0 24%,transparent 24%),'
  'linear-gradient(225deg,rgba(255,255,255,.13) 0 24%,transparent 24%),'
  'linear-gradient(135deg,transparent 0 46%,rgba(0,0,0,.24) 46% 62%,transparent 62%),'
  'linear-gradient(225deg,transparent 0 46%,rgba(0,0,0,.24) 46% 62%,transparent 62%),'
  + FUERTE),

 ('5 · placa con marco interior blanco',
  f'linear-gradient(168deg,{c} 0%,{t(A,-.34)} 100%)'),

 ('6 · degradé fuerte + corte vertical',
  f'linear-gradient(90deg,transparent 0 63%,rgba(255,255,255,.10) 63% 64.4%,'
  f'rgba(0,0,0,.20) 64.4% 100%),'
  + FUERTE),
]

# el 5 lleva el marco interior como capa aparte
CAPAS = {4: ('box-shadow:inset 0 0 0 9px rgba(255,255,255,.92),'
             'inset 0 0 0 11px rgba(0,0,0,.40)')}


def carta(i, etq, fondo):
    cap = CAPAS.get(i, 'display:none')
...
.cap{{position:absolute;inset:0;z-index:1;clip-path:{ESCUDO}}}
...
async def main():
    ...
    cuerpo = ''.join(carta(i, e, f) for i, (e, f) in enumerate(FONDOS))
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           + fuentes + CSS + '</style></head><body>'
           '<div class="rot">DRA · SEIS FONDOS <span>— la corona, el manto, la placa. '
           'Realeza moderna, no ornamental como Fontana.</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'dra.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'dra.png'), full_page=True)
        await b.close()
    print('-> dra.png')
