"""URBF · 2b vs 2c, y el cruce entre los dos.

Faltaba probar lo obvio: las TRES capas del 2c con el CANTO BLANCO del 2b,
que es lo que hace que se lean como pegatinas y no como manchas de tono.
"""
...
FONDOS = [
 ...
 ('2d · las tres capas CON canto blanco',
  f'linear-gradient(100deg,{t(A,-.50)} 0 21.2%,#FFFFFF 21.2% 22.1%,transparent 22.1%),'
  f'linear-gradient(-80deg,{t(A,.26)} 0 19.2%,#FFFFFF 19.2% 20.1%,transparent 20.1%),'
  f'linear-gradient(188deg,transparent 0 65.2%,#FFFFFF 65.2% 66%,{t(A,-.34)} 66% 100%),'
  'linear-gradient(180deg,rgba(255,255,255,.09) 0 34%,transparent 40%),' + BASE_G),

 ('2e · tres capas, canto más grueso',
  ...
  f'linear-gradient(188deg,transparent 0 64.6%,#FFFFFF 64.6% 66.2%,{t(A,-.36)} 66.2% 100%),'
  + BASE_G),
]
...
.fila{{display:flex;gap:38px;flex-wrap:wrap}}
.col{{text-align:center}}
.et{{color:#B0B0C4;font-size:12.5px;letter-spacing:1.3px;margin-bottom:16px;text-transform:uppercase}}
...
.marco{{position:absolute;inset:0;clip-path:{ESCUDO};background:{t(A,-.66)};
  filter:drop-shadow(0 13px 28px rgba(0,0,0,.72))}}
.panel{{position:absolute;inset:5px;clip-path:{ESCUDO};overflow:hidden;
  box-shadow:inset 0 0 0 1.8px rgba(255,255,255,.24)}}
...
.foto{{position:absolute;top:12.5%;left:50%;transform:translateX(-50%);width:36%;height:23%;
  border-radius:14px;overflow:hidden;z-index:4;border:2.8px solid #fff;
...
.num{{font-family:'Archivo','LigaEmoji',sans-serif;font-size:2.3rem;font-weight:900;color:#fff;line-height:.82;
  text-shadow:0 2px 5px rgba(0,0,0,.9)}}
.rng{{font-family:'Archivo','LigaEmoji',sans-serif;font-size:.86rem;font-weight:900;color:#fff;line-height:1;
  margin-top:2px;text-shadow:0 1px 3px rgba(0,0,0,.9)}}
.ban{{width:25px;height:18px;border-radius:3px;object-fit:cover;margin-top:6px;
...
.rk span{{display:flex;align-items:baseline;gap:3px;background:rgba(0,0,0,.46);
  border:1px solid rgba(255,255,255,.30);border-radius:5px;padding:1px 6px 2px}}
.rk b{{font-family:'Archivo','LigaEmoji',sans-serif;font-size:.66rem;font-weight:900;color:#fff}}
.rk u{{font-family:'Archivo','LigaEmoji',sans-serif;text-decoration:none;font-size:.27rem;font-weight:900;
  letter-spacing:.6px;opacity:.62;color:#fff}}
.nom{{position:absolute;top:39%;left:0;right:0;text-align:center;z-index:5;
  font-family:'Archivo','LigaEmoji',sans-serif;font-size:1.52rem;font-weight:900;color:#fff;
  text-shadow:0 2px 4px rgba(0,0,0,.9),0 4px 16px rgba(0,0,0,.7)}}
.regla{{position:absolute;top:46%;left:14%;right:14%;height:1.6px;z-index:5;opacity:.7;
...
.stats b{{font-family:'Barlow Condensed','LigaEmoji',sans-serif;font-size:1.28rem;font-weight:700;color:#fff;
  text-shadow:0 2px 4px rgba(0,0,0,.9)}}
.stats u{{font-family:'Archivo','LigaEmoji',sans-serif;text-decoration:none;font-size:.4rem;font-weight:900;
...
async def main():
    ...
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           ...
           '<div class="rot">URBF · 2b vs 2c, y el cruce <span>— el canto blanco es lo que '
           'las hace leer como pegatina</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'urbf2.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'urbf2.png'), full_page=True)
        await b.close()
    print('-> urbf2.png')
