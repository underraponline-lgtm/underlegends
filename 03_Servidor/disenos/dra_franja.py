"""DRA · oxido + franja al pecho. Ocho variaciones de la franja.

Todo lo demas identico: mismo color, mismo degrade, misma textura de oxido.
Lo unico que cambia es la franja.
"""
...
Bm = 'rgba(255,255,255,.55)'

FRANJAS = [
 ('1 · gruesa + fina (la actual)',
  f'linear-gradient(180deg,transparent 0 44%,{B} 44% 46.4%,transparent 46.4% 47.6%,'
  f'{Bm} 47.6% 48.6%,transparent 48.6%)'),

 ('2 · una sola gruesa',
  f'linear-gradient(180deg,transparent 0 44%,{B} 44% 47.4%,transparent 47.4%)'),

 ('3 · tres finas',
  f'linear-gradient(180deg,transparent 0 43.4%,{B} 43.4% 44.5%,transparent 44.5% 45.8%,'
  f'{B} 45.8% 46.9%,transparent 46.9% 48.2%,{B} 48.2% 49.3%,transparent 49.3%)'),

 ('4 · gruesa que se desvanece',
  'linear-gradient(90deg,transparent 0%,rgba(255,255,255,.95) 22%,'
  'rgba(255,255,255,.95) 78%,transparent 100%) 0 45.4%/100% 3.1% no-repeat'),

 ('5 · fina arriba, gruesa abajo',
  f'linear-gradient(180deg,transparent 0 43.6%,{Bm} 43.6% 44.6%,transparent 44.6% 45.8%,'
  f'{B} 45.8% 48.6%,transparent 48.6%)'),

 ('6 · banda ancha translúcida',
  f'linear-gradient(180deg,transparent 0 43%,rgba(255,255,255,.20) 43% 50%,transparent 50%),'
  f'linear-gradient(180deg,transparent 0 43%,{B} 43% 43.9%,transparent 43.9% 49.1%,'
  f'{B} 49.1% 50%,transparent 50%)'),

 ('7 · dos gruesas separadas',
  f'linear-gradient(180deg,transparent 0 43%,{B} 43% 45.2%,transparent 45.2% 48.4%,'
  f'{B} 48.4% 50.6%,transparent 50.6%)'),

 ('8 · gruesa interrumpida al centro',
  f'linear-gradient(90deg,{B} 0 34%,transparent 34% 66%,{B} 66% 100%) 0 45.2%/100% 3% no-repeat'),
]


def carta(etq, franja):
    return f"""<div class="col"><div class="et">{etq}</div>
<div class="wrap"><div class="marco"><div class="panel">
  <div class="fondo"></div>
  <div class="tex"></div>
  <div class="franja" style="background:{franja}"></div>
...
.franja{{position:absolute;inset:0;z-index:2}}
...
.stats{{position:absolute;top:53%;left:9%;right:9%;z-index:5;
...
async def main():
    ...
    cuerpo = ''.join(carta(e, f) for e, f in FRANJAS)
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           + fuentes + CSS + '</style></head><body>'
           '<div class="rot">DRA · ÓXIDO + FRANJA AL PECHO <span>— ocho variaciones. '
           'Todo lo demás idéntico.</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'dra_franja.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'dra_franja.png'), full_page=True)
        await b.close()
    print('-> dra_franja.png')
