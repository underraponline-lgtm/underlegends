"""DRA · oxido vs esmerilado, con y sin VIVOS.

La linea de Snake Rap marca el borde de un corte: separa dos zonas. Si DRA
usa lo mismo, quedan parientes.

En una camiseta hay otra linea que si es propia: el VIVO. El ribete del
cuello, el puno, la franja al pecho. No separa nada, REMATA la prenda. Una
camiseta lisa con textura casi siempre lo lleva.

Cuatro tratamientos por textura.
"""
...
# los cuatro tratamientos de vivo, como capa encima de la textura
VIVOS = {
 'sin vivo': '',
 'vivo en el borde':
   'box-shadow:inset 0 0 0 6px rgba(255,255,255,.90),'
   'inset 0 0 0 8px rgba(0,0,0,.45)',
 'franja al pecho':
   'background:linear-gradient(180deg,transparent 0 44%,rgba(255,255,255,.92) 44% 46.4%,'
   'transparent 46.4% 47.6%,rgba(255,255,255,.55) 47.6% 48.6%,transparent 48.6%)',
 'vivo + franja baja':
   'box-shadow:inset 0 0 0 5px rgba(255,255,255,.85),inset 0 0 0 7px rgba(0,0,0,.40);'
   'background:linear-gradient(180deg,transparent 0 74%,rgba(255,255,255,.75) 74% 76%,'
   'transparent 76%)',
}

TEXTURAS = [('óxido', 'oxido', 'overlay', .38),
            ('esmerilado', 'esmerilado', 'soft-light', .55)]


def carta(nt, textura, modo, op, nv, vivo):
    return f"""<div class="col"><div class="et">{nt} · {nv}</div>
...
.fila{{display:flex;gap:30px;flex-wrap:wrap;margin-bottom:14px}}
.col{{text-align:center}}
.et{{color:#B0B0C4;font-size:11.5px;letter-spacing:1.2px;margin-bottom:13px;text-transform:uppercase}}
.rot{{color:#EDEDF5;font-size:15px;font-weight:700;letter-spacing:1.3px;margin:8px 0 18px}}
...
.vivo{{position:absolute;inset:0;z-index:2;clip-path:{ESCUDO}}}
...
.stats{{position:absolute;top:52%;left:9%;right:9%;z-index:5;
...
async def main():
    ...
    filas = ''
    for nt, textura, modo, op in TEXTURAS:
        cols = ''.join(carta(nt, textura, modo, op, nv, v) for nv, v in VIVOS.items())
        filas += f'<div class="fila">{cols}</div>'
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           + fuentes + CSS + '</style></head><body>'
           '<div class="rot">DRA · ÓXIDO Y ESMERILADO, CON Y SIN VIVOS '
           '<span>— el vivo no separa zonas como la línea de Snake Rap: remata la prenda</span></div>'
           + filas + '</body></html>')
    out = os.path.join(SCR, 'dra_vivos.html')
    open(out, 'w', encoding='utf-8').write(pag)
    async with async_playwright() as p:
        b = await p.chromium.launch(args=['--no-sandbox'])
        pg = await b.new_page(viewport={'width': 1420, 'height': 1100}, device_scale_factor=2)
        await pg.goto('file://' + out); await pg.wait_for_timeout(2400)
        await pg.screenshot(path=os.path.join(SCR, 'dra_vivos.png'), full_page=True)
        await b.close()
    print('-> dra_vivos.png')
