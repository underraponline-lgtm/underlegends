...
EL PROBLEMA DE CADA UNO, visto con los nueve juntos:
  FRZ  es la unica CLARA del set y rompe la familia. Las otras ocho son
       medias u oscuras.
  FFA  quedo muy cerca de TWR en valor: las dos medias-oscuras con un color
       saturado. Se distinguen por matiz, no por estructura.

FRZ es hielo pero puede ser hielo PROFUNDO: el fondo de un glaciar, no la
superficie. FFA es neon: puede irse mucho mas oscuro y saturado.
"""
...
# ══ FRZ · hielo PROFUNDO, no superficie ══
FA, FB = '#0E5F5E', '#8FE8E0'
FRZ = [
 ('1 · el actual · claro arriba',
  'linear-gradient(122deg,rgba(255,255,255,.24) 0 26%,transparent 26%),'
  'linear-gradient(-138deg,rgba(0,0,0,.26) 0 30%,transparent 30%),'
  f'linear-gradient(196deg,{t("#CFFAF4",-.10)} 0 38%,{t("#178C88",-.72)} 38% 40%,transparent 40%),'
  f'linear-gradient(166deg,{t("#178C88",.12)} 0%,{t("#178C88",-.60)} 100%)', '', ''),

 ('2 · glaciar profundo · claro solo arriba',
  f'linear-gradient(196deg,{t(FB,-.16)} 0 22%,{t(FA,-.60)} 22% 24%,transparent 24%),'
  f'linear-gradient(168deg,{t(FA,.20)} 0%,{t(FA,-.35)} 46%,{t(FA,-.78)} 100%)', '', ''),

 ('3 · hielo craquelado',
  f'linear-gradient(168deg,{t(FA,.10)} 0%,{t(FA,-.45)} 52%,{t(FA,-.82)} 100%)',
  f'background-image:url({tx("craquelado")});background-size:cover;'
  'mix-blend-mode:screen;opacity:.24', ''),

 ('4 · grietas de hielo',
  f'linear-gradient(168deg,{t(FA,.16)} 0%,{t(FA,-.40)} 50%,{t(FA,-.80)} 100%)',
  f'background-image:url({tx("grietas")});background-size:cover;'
  'mix-blend-mode:screen;opacity:.34', ''),

 ('5 · vidrio esmerilado oscuro',
  f'linear-gradient(168deg,{t(FA,.22)} 0%,{t(FA,-.42)} 48%,{t(FA,-.80)} 100%)',
  f'background-image:url({tx("esmerilado")});background-size:cover;'
  'mix-blend-mode:soft-light;opacity:.55', ''),

 ('6 · témpano espejado + oscuro',
  f'linear-gradient(122deg,rgba(255,255,255,.14) 0 24%,transparent 24%),'
  f'linear-gradient(214deg,{t(FB,-.24)} 0 30%,{t(FA,-.78)} 30% 32%,transparent 32%),'
  f'linear-gradient(168deg,{t(FA,.06)} 0%,{t(FA,-.50)} 52%,{t(FA,-.84)} 100%)', '', ''),
]

# ══ FFA · neon, mucho mas oscuro y saturado ══
GA, GB = '#1A0630', '#EC48DC'
FFA = [
 ('1 · el actual',
  'radial-gradient(ellipse 70% 40% at 50% 28%,rgba(236,72,220,.34),transparent 62%),'
  f'linear-gradient(168deg,{t(GA,.28)} 0%,{GA} 52%,#0C0518 100%)', '', ''),

 ('2 · negro casi puro + neón fuerte',
  'radial-gradient(ellipse 62% 32% at 50% 26%,rgba(236,72,220,.52),transparent 60%),'
  'linear-gradient(168deg,#1C0736 0%,#0B0418 54%,#050209 100%)', '', ''),

 ('3 · humo + neón',
  'radial-gradient(ellipse 62% 34% at 50% 26%,rgba(236,72,220,.44),transparent 62%),'
  'linear-gradient(168deg,#1A0630 0%,#0A0316 100%)',
  f'background-image:url({tx("humo")});background-size:cover;'
  'mix-blend-mode:screen;opacity:.20', ''),

 ('4 · letrero de neón · líneas',
  'linear-gradient(168deg,#170528 0%,#080312 100%)',
  '', 'background:'
  'linear-gradient(180deg,transparent 0 30%,rgba(236,72,220,.85) 30% 31.4%,transparent 31.4%),'
  'linear-gradient(180deg,transparent 0 66%,rgba(236,72,220,.55) 66% 67.2%,transparent 67.2%),'
  'linear-gradient(180deg,transparent 0 84%,rgba(236,72,220,.35) 84% 85%,transparent 85%);'
  'box-shadow:inset 0 0 40px rgba(236,72,220,.28)'),

 ('5 · estática + neón',
  'radial-gradient(ellipse 66% 34% at 50% 28%,rgba(236,72,220,.46),transparent 62%),'
  'linear-gradient(168deg,#190630 0%,#090315 100%)',
  f'background-image:url({tx("estatica")});background-size:cover;'
  'mix-blend-mode:soft-light;opacity:.16', ''),

 ('6 · doble neón · magenta y violeta',
  'radial-gradient(ellipse 54% 28% at 26% 22%,rgba(236,72,220,.50),transparent 62%),'
  'radial-gradient(ellipse 54% 30% at 78% 62%,rgba(124,58,237,.52),transparent 64%),'
  'linear-gradient(168deg,#150524 0%,#06020C 100%)', '', ''),
]


def carta(sv, ac, etq, fondo, cap, extra):
    c1 = f'<div class="cap" style="{cap}"></div>' if cap else ''
    c2 = f'<div class="rem" style="{extra}"></div>' if extra else ''
    return f"""<div class="col"><div class="et">{etq}</div>
<div class="wrap"><div class="marco" style="background:{t(ac,-.72)}"><div class="panel">
...
async def main():
    ...
    f1 = ''.join(carta('FRZ', '#8FE8E0', *c) for c in FRZ)
    f2 = ''.join(carta('FFA', '#EC48DC', *c) for c in FFA)
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           + fuentes + CSS + '</style></head><body>'
           '<div class="rot">FRZ · HIELO PROFUNDO <span>— era la única clara del set y '
           'rompía la familia</span></div><div class="fila">' + f1 + '</div>'
           '<div class="rot">FFA · NEÓN MÁS OSCURO <span>— estaba muy cerca de TWR en valor</span></div>'
           '<div class="fila">' + f2 + '</div></body></html>')
    out = os.path.join(SCR, 'frz_ffa.html')
    open(out, 'w', encoding='utf-8').write(pag)
    async with async_playwright() as p:
        b = await p.chromium.launch(args=['--no-sandbox'])
        pg = await b.new_page(viewport={'width': 1060, 'height': 1200}, device_scale_factor=2)
        await pg.goto('file://' + out); await pg.wait_for_timeout(2600)
        await pg.screenshot(path=os.path.join(SCR, 'frz_ffa.png'), full_page=True)
        await b.close()
    print('-> frz_ffa.png')
