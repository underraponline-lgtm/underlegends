"""DRA · textura + gesto, pensado como CAMISETA.

La referencia es un kit de futbol: el patron CUBRE la prenda y se siente
tejido, no pegado encima. Por eso en estas seis la textura no es una capa
decorativa suelta sino que convive con el gesto estructural.
"""
...
RAYOS = ('repeating-conic-gradient(from 202deg at 50% -6%,'
         'rgba(255,255,255,.17) 0deg 4deg,transparent 4deg 11deg) 0 0/100% 54% no-repeat,')
DIAG = (f'linear-gradient(146deg,transparent 0 52%,rgba(255,255,255,.16) 52% 55%,'
        f'rgba(0,0,0,.30) 55% 100%),')
BANDA = (f'linear-gradient(122deg,transparent 0 46%,rgba(255,255,255,.22) 46% 52%,'
         f'rgba(255,255,255,.07) 52% 66%,transparent 66%),')

# (etiqueta, fondo css, textura, modo, opacidad)
CASOS = [
 ('craquelado + rayos de corona', RAYOS + FUERTE, 'craquelado', 'screen', .22),
 ('craquelado solo, fuerte',      FUERTE,         'craquelado', 'screen', .34),
 ('cuero + rayos',                RAYOS + FUERTE, 'cuero',      'soft-light', .50),
 ('craquelado + banda al sesgo',  BANDA + FUERTE, 'craquelado', 'screen', .20),
 ('tela + rayos · tejido de kit', RAYOS + FUERTE, 'tela',       'soft-light', .38),
 ('craquelado + corte diagonal',  DIAG + FUERTE,  'craquelado', 'screen', .24),
]


def carta(etq, fondo, textura, modo, op):
...
async def main():
    ...
    pag = ('<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><style>'
           ...
           '<div class="rot">DRA · TEXTURA + GESTO <span>— pensado como camiseta: el patrón '
           'cubre la prenda, no está pegado encima</span></div>'
           '<div class="fila">' + cuerpo + '</div></body></html>')
    out = os.path.join(SCR, 'dra_combo.html')
    ...
    async with async_playwright() as p:
        ...
        await pg.screenshot(path=os.path.join(SCR, 'dra_combo.png'), full_page=True)
        await b.close()
    print('-> dra_combo.png')
